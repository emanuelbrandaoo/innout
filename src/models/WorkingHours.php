...
use WorkingHours as GlobalWorkingHours;
...
class WorkingHours extends Model {
    ...
    public function innout($time) {
        ...
        if(!$timeColumn) {
            throw new AppException("Você já bateu os 4 pontos do dia.");
        }
    ...
    function getLunchBreak() {
        [, $t2, $t3,] = $this->getTimes();
        $lunchBreak = new DateInterval('PT0S');

        if($t2) $lunchBreak = $t2->diff(new DateTime());
        if($t3) $lunchBreak = $t2->diff($t3);
    ...
    function getExitTime() {
        ...
        $defaultlunchBreak = DateInterval::createFromDateString('1 hour');

        if(!$t1) {
            return (new DateTimeImmutable())->add($workday)->add($defaultlunchBreak);
        } elseif ($t4) {
            return $t4;
        } else {
            $total = sumIntervals($workday, $this->getLunchBreak());
            return $t1->add($total);
        }
    }
